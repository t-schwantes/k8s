...
echo "Copying reset-k8s.sh to worker node $WORKER_NODE_IP..."
...
ssh "$WORKER_USER@$WORKER_NODE_IP" bash -s <<EOF
    echo "Making reset-k8s.sh executable and running it on the worker node..."
    chmod +x $REMOTE_SCRIPT_DIR/reset-k8s.sh
    sudo $REMOTE_SCRIPT_DIR/reset-k8s.sh
EOF
